...
/*
    ...
    *** 정규 표현식 객체가 제공하는 메서드(함수) ***
    1. regEx.test(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면 true, 아니면 false 반환

    2. regEx.exec(문자열)
        -> 문자열이 정규표현식 패턴에 부합하면
            첫 번째 매칭되는 문자열을 반환, 없으면 null 반환
    ...
    *** 정규 표현식의 메타 문자 ***
    ...
    a (일반문자열) : 문자열 내에 a라는 문자열이 존재하는 지 검색 
...
//     obj.c = 3 // d-> {"a":1, "b":2, "c":3}

// 3) 객체의 특정 Key를 삭제할 수 있다. (delete 연산자)
// ex) const obj = {"a":1, "b":2}
//     delete obj.b; // {"a":1}
...
memberEmail.addEventListener("input", () =>{
    ...
    if(memberEmail.value.trim().length == 0){
        ...
        memberEmail.value="";
        emailMessage.classList.remove("confirm","error"); // .confirm .error 스타일 제거
    ...
    if( regEx.test(memberEmail.value)){ // 유효한 경우
        /******************************************/
        /* fetch() API를 이용한 ajax(비동시 통신) */
        /******************************************/
        fetch('/dupCheck/email?email=' + memberEmail.value)
        .then(resp=>resp.text()) // 응답객체 -> 파싱
...
memberPw.addEventListener("input", () => {
    ...
    if(memberPw.value.trim().length==0){
        ...
        memberPw.classList.remove("confirm","error"); // 검정 글씨
...
document.getElementById("signUpFrm").addEventListener("submit",e=>{
    ...
    // (배열용 for문)
    // *** for ... of : 향상된 for문
    //         -> iterator 속성을 지닌 배열, 유사 배열 사용 가능

    // (객체용 for문)
    // *** for ... in 구문 ***
    //         -> JS 객체가 가지고 있는 key를 순서대로 하나씩 꺼내는 반복문
    for(let key in checkObj){
        if(!checkObj[key]){ // 각 key에 대한 value(true/false)를 얻어 와
                            // false인 경우 == 유효하지 않다!
            switch(key){
