...
SELECT MAX(SUM(SALARY)) FROM EMPLOYEE E GROUP BY DEPT_CODE ;
...
GROUP BY DEPT_TITLE 
HAVING SUM(SALARY) = (SELECT MAX(SUM(SALARY)) FROM EMPLOYEE E2 GROUP BY DEPT_CODE );                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
...
FROM EMPLOYEE E 
WHERE SALARY IN (SELECT MAX(SALARY) FROM EMPLOYEE E2 GROUP BY DEPT_CODE)
ORDER BY DEPT_CODE;
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME,
	CASE
		WHEN EMP_ID IN(SELECT DISTINCT MANAGER_ID FROM EMPLOYEE E WHERE MANAGER_ID  IS NOT NULL)
		THEN '사수'
		ELSE '사원'
	END AS 구분
FROM EMPLOYEE E2 
...
SELECT E.EMP_ID , EMP_NAME , DEPT_CODE , JOB_CODE , D.DEPT_TITLE , J.JOB_NAME 
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
NATURAL JOIN JOB J
WHERE (DEPT_TITLE, JOB_NAME) = (SELECT D.DEPT_TITLE , J.JOB_NAME
								FROM EMPLOYEE E
								JOIN DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
								NATURAL JOIN JOB J
...
-- EXITSTS : 서브쿼리에서 조회된 결과와 일치하는 행이
--			 메인 쿼리에 하나라도 있으면 TRUE(조회 결과에 포함)
--메인 쿼리의 각 행마다 메인 -> 서브 -> 메인
...
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE)
WHERE (HIRE_DATE,NVL(DEPT_CODE,0)) IN(SELECT MIN(HIRE_DATE),NVL(DEPT_CODE,0)  FROM EMPLOYEE
				   WHERE ENT_YN = 'N'
				   GROUP BY DEPT_CODE);
...
WHERE HIRE_DATE =(
		...
		WHERE ENT_YN = 'N' AND (SUB.DEPT_CODE = MAIN.DEPT_CODE OR
		(MAIN.DEPT_CODE IS NULL AND SUB.DEPT_CODE IS NULL)))
--		WHERE (ENT_YN = 'N' AND SUB.DEPT_CODE = MAIN.DEPT_CODE) OR 
--	  (ENT_YN = 'N' AND SUB.DEPT_CODE IS NULL AND MAIN.DEPT_CODE IS NULL))
--		WHERE ENT_YN = 'N' AND NVL(SUB.DEPT_CODE,'0') = NVL(MAIN.DEPT_CODE,'0'))
ORDER BY HIRE_DATE;
...
WHERE ROWNUM <= 5
ORDER BY SALARY DESC;
--> EMPLOYEE 테이블 위에서 5명을 가지고 급여 내림차순 조회
-- 선동일 송종기 정중하 대북혼 노옹철

-- 문제 원인 : SELECT 해석 순서상
--			ORDER BY가 가장 마지막으로 해석되기 떄문에
--			급여 상위 5명이라는 의도와 맞지 않는 형태로 조회됨

-- 해결 방법 : 인라인뷰(FROM절에 작성되는 서브쿼리)
-- 해결 1) 급여 내림차순 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE E 
ORDER BY SALARY DESC;
...
-- 급여 평균이 590만인 부서를 조회
SELECT DEPT_CODE, DEPT_TITLE, 평균, ROWNUM FROM
	(SELECT DEPT_CODE, DEPT_TITLE, FLOOR(AVG(SALARY)) 평균
	FROM EMPLOYEE E2 
	LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
	GROUP BY DEPT_CODE, DEPT_TITLE
