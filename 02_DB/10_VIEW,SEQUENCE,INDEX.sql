...
/* ***************** 관리자에게 VIEW생성 권한 부여받아야 가능 ****************** */
CREATE VIEW V_EMP
AS SELECT EMP_NO, EMP_NAME , DEPT_TITLE, JOB_NAME
FROM EMPLOYEE
NATURAL JOIN JOB
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID);
...
AS SELECT EMP_NO 사번, EMP_NAME 이름, DEPT_TITLE 부서명, JOB_NAME 직급명
FROM EMPLOYEE
NATURAL JOIN JOB
LEFT JOIN DEPARTMENT ON(DEPT_CODE=DEPT_ID);
...
DEPT_TITLE CONSTRAINT NN_DEPT_TITLE NOT NULL;

-- 다시 VIEW를 이용해서 INSERT
INSERT INTO V_DCOPY2 VALUES('D0', 'L3');
--> 원본 테이블 DEPT_ID, LOCATION_ID 컬럼에 'D0', 'L3' 삽입
---> DEPT_TITLE 컬럼에 NULL 삽입
---> 그런데 DEPT_TITLE에 NOT NULL 제약조건이 설정되어
--   NULL 값이 삽입될 수 없음.

--ORA-01400: NULL을 ("KH_LDH"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- ORA-08002: 시퀀스 SEQ_TEST_NO.CURRVAL은 이 세션에서는 정의 되어 있지 않습니다
...
-- 시퀀스는 STARTWITH 를 변경하고 싶을 때 삭제 후 다시 생성해야 한다.
 ...
 * - 조회시 테이블의 전체 내용을 확인하며 조회하는 것이 아닌
 ...
 *
 * - INDEX도 하나의 객체이다 보니 별도 저장 공간이 필요(메모리 소비)
 * 
 * - INDEX 생성 시간이 필요함.
 * 
 * [작성법]
...
-- 인덱스 사용 X 조회
SELECT * FROM TB_INDEX_TEST WHERE TEST_ID = 'TEST500000';

-- 인덱스 사용 O 조회
SELECT * FROM TB_INDEX_TEST WHERE TEST_NO = 500000;
