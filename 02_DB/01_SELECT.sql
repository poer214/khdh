 ...
 *	-> 조회된 행들의 집함을 "RESULT SET"이라고 한다.
 *
 * - RESULT SET은 0개 이상의 행이 포함될 수 있다.
 * 	-> 왜 0개이상?? 조건에 맞는 행이 없을 수도 있기 때문에.
 * 
 * [작성법]
 * 
 * SELECT 컬럼명 |(또는) *(모든 컬럼)
...
-- EMPLOYEE 테이블에서 모든 모든 사원이 사번, 이름, 급여, 급여+100만을 조회
...
-- <컬럼 별칭 지정>
-- 1) 컬렴명 AS 별칭 : 문자O, 띄어쓰기X, 특수문자X
-- 2) 컬럼명 AS "별칭" : 문자O, 띄어쓰기O, 특수문자O
-- 3) 컬렴명 별칭 : 문자O, 띄어쓰기X, 특수문자X
-- 3) 컬렴명 "별칭" : 문자O, 띄어쓰기O, 특수문자O

-- EMPLOYEE 테이블에서 이름, 연봉 조회
SELECT EMP_NAME AS "[사원 이름]", SALARY * 12
FROM EMPLOYEE;
...
-- <WHERE 절>
-- 테이블에서 조건을 충족하는 컬럼 값을 가진 행만 조회할 떄 사용
...
-- EMPLOYEE 테이블에서
-- 부서 코드가 'D5' 또는 'D6' 또는 'D9'이 아닌 사원의
-- 사번, 이름, 부서코드 조회
...
-- NOT IN (값1, 값2, 값3, ...) : 컬럼 값에 () 안의 값을 제외한 값이 TRUE
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- 이메일에 _ 앞 글자가 3글자인 사원의
-- 사번, 이름, 이메일 조회
SELECT EMP_ID, EMP_NAME, EMAIL 
FROM EMPLOYEE
WHERE EMAIL LIKE '___%';
--> 문제점 : 와일드 카드 문자(_)와 패턴에 사용된 일반 문자가
--		  같은 문자이기 떄문에 구분이 안되는 문제가 발생

--> 해결 방법 : ESCAPE 옵션을 사용하여 일반 문자(_)를 구분
SELECT EMP_ID, EMP_NAME, EMAIL 
FROM EMPLOYEE
...
SELECT EMP_NAME , SALARY FROM EMPLOYEE WHERE SALARY >= 2000000 ORDER BY SALARY;
...
ORDER BY PHONE /*NULLS LAST*/ NULLS FIRST;
		-- 오름차순에서 NULLS LAST가 기본값.

-- 전화번호 내림차순 조회
SELECT EMP_NAME, PHONE
FROM EMPLOYEE e 
ORDER BY PHONE DESC /*NULLS FIRST*/ NULLS LAST;
		-- 내림차순에서 NULLS FIRST가 기본값.
