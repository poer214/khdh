...
DEPT_TITLE NOT NULL; -- NULL관련 설정만 MODIFY 사용!!(기억!)

---------------------------

-- DEPT_COPY에 추가한 제약조건 중 PK 빼고 모두 삭제
-- DEPT_COPY 에 추가한 제약조건 중 PK 뺴고 모두 삭제
-- ALTER 

ALTER TABLE DEPT_COPY DROP CONSTRAINT CHK_DEPT_COPY;
ALTER TABLE DEPT_COPY DROP CONSTRAINT UK_DEPT_COPY;
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007557;
--> NOT NULL 제약조건 ADD는 안되는데 DROP은 가능

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE CONSTRAINT SYS_007557 NOT NULL;
...
ALTER TABLE DEPT_COPY DROP (NAME);
...
SELECT * FROM DEPT_COPY; -- 'D0' 삽입된 것 확인
ROLLBACK; -- 트랜잭션에서 'D0' INSERT 내용을 삭제
SELECT * FROM DEPT_COPY; -- 'D0' 사라진 것 확인


INSERT INTO DEPT_COPY VALUES('D0'); -- 'D0' 삽입
SELECT * FROM DEPT_COPY; -- 다시 삽입된 것 확인

ALTER TABLE DEPT_COPY MODIFY DEPT_ID VARCHAR2(4); -- DDL 수행

ROLLBACK; -- 트랜잭션에서 'D0' 다시 지우고싶은데 !
