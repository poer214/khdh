...
SELECT EMP.EMP_ID, EMP.EMP_NAME,
	NVL(EMP.MANAGER_ID,'없음') 사수사번, NVL(MGR.EMP_NAME,'없음') 사수이름
FROM EMPLOYEE EMP
...
--> NATURAL JOIN했는데 CROSS JOIN결과가 나왔다? -> 같은 컬럼이 없다.
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
-- LOCATION을 먼저 조인하려고 하면 LOCATION ID가 없다.
...
SELECT EMP_NAME , E.EMP_NO , D.DEPT_TITLE , J.JOB_NAME 
FROM EMPLOYEE e
...
--연봉에 보너스포인트를 적용하시오.
SELECT E.EMP_NAME , J.JOB_NAME, E.SALARY , (E.SALARY*(1+NVL(E.BONUS,0)))*12 연봉
FROM EMPLOYEE e 
NATURAL JOIN JOB
...
-- SELF JOIN 사용
SELECT E.EMP_NAME 사원명, DEPT_CODE 부서코드, E2.EMP_NAME 동료이름
FROM EMPLOYEE e 
