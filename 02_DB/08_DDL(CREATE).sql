...
-- DESC문 : 테이블의 구조를 표시
DESC MEMBER; -- 구버전에서 사용됐었는데 현재 사용 불가
...
ROLLBACK; --> 트랜잭션에 저장된 DML 삭제하고
		  --> 마지막 COMMIT 상태로 돌아감

SELECT TO_DATE('20230316')-(3*24)
FROM DUAL;
...
-- ORA-01400: NULL을 ("KH_LDH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
--ORA-00001: 무결성 제약 조건(KH_LDH.SYS_C007758)에 위배됩니다
--ORA-00001: 무결성 제약 조건(KH_LDH.USER_ID_UK)에 위배됩니다
...
AND UCC.CONSTRAINT_NAME = 'USER_ID_IK';
...
--    USER_NO NUMBER PRIMARY KEY, -- 컬럼 레벨(제약조건명 미지정)
--    USER_NO NUMBER CONSTRAINT USER_NUMBER_PK PRIMARY KEY,
    							  -- 컬럼 레벨(제약조건명 지정)
	USER_NO NUMBER,    
...
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할 테이블 명 [(참조)]
...
--ORA-02291: 무결성 제약조건(KH_LDH.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
CREATE TABLE USER_TEST(
	...
	PNO VARCHAR2(16) CONSTRAINT NN_PNO NOT NULL
