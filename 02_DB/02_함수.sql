...
-- 단힐행 함수 : N개의 행의 값을 전달하여 N개의 결과를 반환
...
-- 문자열을 5번째 부터 검색하여 두 번째로 찾는 B의 위치 조회
...
-- 2023-03-07 12:22:47.000, 2023-03-07 12:22:47.883 +0900
...
-- 문자열(CHAR) <-> 날짜(DATE)
-- 숫자(NUMBER) --> 날짜(DATE)
--> 숫자는 날짜가 될 수 있지만, 날짜는 숫자가 될 수 없다.
...
SELECT '2023-03-07', TO_DATE('2023년 03월 07일','YYYY"년" MM"월" DD"일"')
FROM DUAL;
...
/* 날짜 패턴 Y, R의 차이점 */
-- 연도를 짧게 해석하는 경우
-- 50미만 : 앞부분에 현재 세기를 적용(21C == 2000년대)를 적용
-- 50이상 : Y == 현재 세기(2000년대)
--		  R == 이전 세기(1900년대)
...
-- 기존에 보너스를 받았던 사원은 기존 보너스 + 0.2로 변경
...
-- EMPLOYEE테이블에서 모든 사원의 사번, 이름, 성별 조회하기
SELECT EMP_ID, EMP_NAME,
	DECODE(SUBSTR(EMP_NO,8,1),'1','남자','2','여자','그 외')
FROM EMPLOYEE;

-- EMPLOYEE테이블에서
-- 직급 코드가 'J3'인 직원은 급여 + 급여의 10%
-- 직급 코드가 'J6'인 직원은 급여 + 급여의 15%
-- 직급 코드가 'J5'인 직원은 급여 + 급여의 20%
...
SELECT	COUNT(DECODE(SUBSTR(EMP_NO,8,1),'1',1,NULL)) "남자 사원 수",
		...
		SUM(DECODE(SUBSTR(EMP_NO,8,1),'1',1,0)) "남자 사원 수",
