...
public class SetService {
	...
	public void ex2() {
		...
		// - Hash가 붙은 이유 : 속도 향상
		//  -> hashCode() 오버라이딩 필수
		//  --> equals()가 true일 때 hashCode()도 같아야 한다.
...
//			Set<Integer> set = new LinkedHashSet<>(); // 순서를 유지하는 Set
			Set<Integer> set = new TreeSet<>();	// 이진트리 구조를 이용해
												// 정렬을 지원하는 Set
			 
			// set에 저장된 객체의 수가 6개 미만이면 반복
			// == 6개면 멈추겠다.
			while(set.size()<6) set.add(random.nextInt(45)+1);
			//완성된 한 세트의 로또번호를 List에 추가
			lottoList.add(set);
		}
